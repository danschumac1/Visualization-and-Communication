...
ggplot(mpg2, aes(displ, hwy)) +
  ...
  #this is the new thing notice ~function of class
  facet_wrap(~class)
...
A few things to note here for the default behavior. 1. `class` labels are organized alphabetically 2. `class` labels go from left to right 3. `class` labels start from the top row
