...
```{r}
p2 +
          # annotate does not look at your data.
  ...
  annotate(geom = "text", label = "Mortgage crisis",
           # These are centering quardinates
...
For annotating plots with images like `geom_text` we have to update the data set with the file path to those images. It's a bit tedious and you should use it only if you really need to. The package for this is `ggimage` and the relevant geom is `geom_image()`. Read more about it here: https://yulab-smu.top/pkgdocs/ggimage.html
...
```{r}
 economics %>% 
  ...
  # raster means png and jpeg files. reads pixle by pixle
  annotation_raster(readPNG("UTSA_Logo_Website_UTSA_Logo_Stacked_Center.png"),
                    
                    #coordinates                                        # looks better when true?
...
```{r}
p2 +            # <b is bold 
                   # style is a css attribute                       # br break line 
...
') +
  #line height makes a bigger gap between the title and the subtitle. 
  theme(plot.title = element_markdown(lineheight = 1.9))
```
...
') +
                      # this is better than element_textbox plain
  theme(plot.title = element_textbox_simple(lineheight = 1.2, fill = "lightsteelblue1", family =                                                # T    R    B    L
                                            padding = margin(5.5, 5.5, 5.5, 5.5)))
```

```{r}
#create dataframe with 1 obs. (where you want to place your label)
text_dt = data.frame(date = as.Date("1990-01-01"), 
                     unemploy_per = 0.051, 
                     mylabel = '<span style = "color: red">**Reagen recession** </span>')

p2 +
  # annotations inside the panel.
